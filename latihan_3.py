...
if check_password():
    ...
    with col_h2:
        st.markdown("""
            ...
            <h4 style='color:gray;'>Jabatan Kejuruteraan Awam - Unit Geomatik</h4>
    ...
    if uploaded_data is not None:
        ...
        if 'E' in df.columns and 'N' in df.columns:
            ...
            ax.scatter(df['E'], df['N'], color='red', s=point_size, marker='o', edgecolors='black', zorder=3)
            ...
            for i in range(num_stn):
                # Label Stesen (Ditambah offset supaya tidak bertindih)
                if show_stn_labels:
                    ...
                    ax.text(x[i] + (dx/dist_c)*label_offset, y[i] + (dy/dist_c)*label_offset, 
                            str(df['STN'].iloc[i]), fontsize=text_size+2, fontweight='bold', 
                ...
                if show_bearing_dist:
                    ...
                    ax.text(mid_e, mid_n, f"{decimal_to_dms(angle)}\n{dist:.3f}m", 
                            fontsize=text_size, color='darkgreen', fontweight='bold',
                            ha='center', va='center', bbox=dict(facecolor='white', alpha=0.7, lw=0))

            if show_area_label:
                ax.text(centroid_x, centroid_y, f"LUAS:\n{area:.3f} m²", 
                        ha='center', va='center', fontsize=text_size+3, fontweight='bold',
                        bbox=dict(facecolor='#ffffcc', alpha=0.9, edgecolor='black', boxstyle='round,pad=0.5'))
            ...
            if show_satelite:
                ...
                try:
                    ...
                    st.error(f"Ralat EPSG: Pastikan kod EPSG {epsg_code} adalah sah untuk koordinat anda.")
